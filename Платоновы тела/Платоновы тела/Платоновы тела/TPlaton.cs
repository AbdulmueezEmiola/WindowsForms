    ...
    public  class TPlaton
    {
        ...
        public void DrawBody(Graphics g)
        {
            ...
            switch (flEdge)
            {
                ...
                case 1:  // faces
                    ...
                    break;
                case 2: // stereo
                    Color c = Color.FromArgb(0xFF,0,0);
                    ...
                    for (int i = 0; i < L; i++)
                    {
                        double[] V1 = Rotate(body.VertexsT[body.Edges[i].p1], 1, AnglStereo, 0, 0);
                        double[] V2 = Rotate(body.VertexsT[body.Edges[i].p2], 1, AnglStereo, 0, 0);
                        g.DrawLine(myPenR, II(V1[0]), JJ(V1[1]), II(V2[0]), JJ(V2[1]));

                        V1 = Rotate(body.VertexsT[body.Edges[i].p1], 1, -AnglStereo, 0, 0);
                        V2 = Rotate(body.VertexsT[body.Edges[i].p2], 1, -AnglStereo, 0, 0);
        ...
        private void DrawPoint(Graphics g)
        {
            ...
            if (Xs!=0)
            {
                ...
                for (int i=0; i<6; i++)
                if ((i!=2) && (i!=3))
                    g.DrawLine(Pens.Silver,u,v,II(body0.VertexsT[i][0]),JJ(body0.VertexsT[i][1]));
        ...
        TEdge[] GetEdges(TFace[] Sides)
        {
            ...
            for (int i=0; i<Ls; i++)
            {
                ...
                for (int j=0; j<L; j++)
                {
                    ...
                    {
                        k++;
                        Ok = ((Result[k].p1 == Sides[i].p[j]) & (Result[k].p2 == Sides[i].p[(j + 1) % L])) |
                           ((Result[k].p2 == Sides[i].p[j]) & (Result[k].p1 == Sides[i].p[(j + 1) % L]));
                    }
                    if (!Ok)
                    {
